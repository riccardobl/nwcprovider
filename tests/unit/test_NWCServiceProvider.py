...
def nwc_service_provider():
    return NWCServiceProvider("d7b5232fba0e02e32cfe26f20cdf2c803b27ecd81052c2dd5d17e5e1a333fe58", "")


@pytest.fixture
def nwc_service_provider2():
    return NWCServiceProvider("ce40821040275f72f3074a89770db3e2744b189f204807c867840eb58565de51", "")
...
async def test_supported_methods(nwc_service_provider):
    def make_invoice(provider, pubkey, content):
        return "invoice"
    nwc_service_provider.addRequestListener("make_invoice", make_invoice)
    s = nwc_service_provider.getSupportedMethods()
...
async def test_signverify(nwc_service_provider, nwc_service_provider2):
    ...
    for _ in range(random.choice([1, 2, 3, 4])):
        tags.append([random.choice(string.ascii_letters)+"_" +
...
async def test_handle(nwc_service_provider, nwc_service_provider2):
    ...
    event = {
        ...
        "content": content,
        "tags": [
            ["p", nwc_service_provider2.public_key_hex]
        ],
        "created_at": 1234567890
    ...
    nwc_service_provider2.addRequestListener(
